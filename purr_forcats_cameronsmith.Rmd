...
author: "Cameron Smith, edited by Jack Wright"
date: "11/7/2020"
...
JACK: The flavor_profile column was not yet set as a factor. I added a line to format it as a factor. 
```{r}
#set flavor profile as a factor
df$flavor_profile<-as.factor(df$flavor_profile)
...
JACK: a column in a data frame is itself a list, so there is no need to first turn the column into a list.

```{r}

# Create an example list
#example_list <- as.list(df$ingredients)
#a data frame is a list of lists, so you don't need to set the column to a list

# Pluck an element by index
pluck(df$ingredients, 1)


# Searching for element that does not exist - pluck returns NULL
pluck(df$ingredients, 100000)

# But chuck returns an error
chuck(df$ingredients, 100000)
```
